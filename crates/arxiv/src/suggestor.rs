...
//! Formation-callable surface — [`ArxivLookupSuggestor`] reads
//! [`ArxivRequest`] facts from `ContextKey::Seeds` and proposes
//! typed [`ArxivPaperPayload`] facts to `ContextKey::Hypotheses`.
//!
...
use crate::provenance::ARXIV_PROVENANCE;
use crate::provider::{ArxivProvider, ArxivRequest};
use crate::types::Paper;

/// Typed fact payload — one [`Paper`] per fact. Flattens the
...
impl FactPayload for ArxivPaperPayload {
    const FAMILY: &'static str = "embassy.arxiv.paper";
    const VERSION: u16 = 1;
}

pub struct ArxivLookupSuggestor<P: ArxivProvider + 'static> {
    provider: Arc<P>,
}

impl<P: ArxivProvider + 'static> ArxivLookupSuggestor<P> {
...
#[async_trait]
impl<P: ArxivProvider + 'static> Suggestor for ArxivLookupSuggestor<P> {
    ...
    fn accepts(&self, ctx: &dyn Context) -> bool {
        ctx.get(ContextKey::Seeds)
            .iter()
            .any(|fact| fact.payload::<ArxivRequest>().is_some())
    }
    ...
    async fn execute(&self, ctx: &dyn Context) -> AgentEffect {
        ...
        for seed in ctx.get(ContextKey::Seeds) {
            let Some(request) = seed.payload::<ArxivRequest>() else {
                continue;
            };
                ...
                .await
            {
                ...
                Err(err) => {
                    tracing::warn!(
                        ...
                        error = %err,
                        "arxiv fetch failed; skipping seed"
                    );
                    continue;
                }
            };
            ...
            for (idx, observation) in response.records.into_iter().enumerate() {
                ...
                let payload_value = ArxivPaperPayload {
                    paper: observation.content,
...
mod tests {
    use super::*;
    use crate::provider::StubArxivProvider;
    ...
    fn payload_family_and_version_are_stable() {
        ...
        assert_eq!(ArxivPaperPayload::FAMILY, "embassy.arxiv.paper");
        assert_eq!(ArxivPaperPayload::VERSION, 1);
    }
}
