...
//! Formation-callable surface — [`WikidataLookupSuggestor`] reads
//! [`WikidataRequest`] facts from `ContextKey::Seeds` and proposes
//! typed [`WikidataEntityPayload`] facts to `ContextKey::Hypotheses`.
//!
...
use crate::provenance::WIKIDATA_PROVENANCE;
use crate::provider::{WikidataProvider, WikidataRequest};
use crate::types::Entity;

/// Typed fact payload — one [`Entity`] per fact. Flattens the
...
pub struct WikidataEntityPayload {
    pub entity: Entity,
...
impl FactPayload for WikidataEntityPayload {
    const FAMILY: &'static str = "embassy.wikidata.entity";
    const VERSION: u16 = 1;
}

pub struct WikidataLookupSuggestor<P: WikidataProvider + 'static> {
    provider: Arc<P>,
}

impl<P: WikidataProvider + 'static> WikidataLookupSuggestor<P> {
...
#[async_trait]
impl<P: WikidataProvider + 'static> Suggestor for WikidataLookupSuggestor<P> {
    fn name(&self) -> &'static str {
        "WikidataLookupSuggestor"
    }
    ...
    fn accepts(&self, ctx: &dyn Context) -> bool {
        ctx.get(ContextKey::Seeds)
            .iter()
            .any(|fact| fact.payload::<WikidataRequest>().is_some())
    }
    ...
    async fn execute(&self, ctx: &dyn Context) -> AgentEffect {
        ...
        for seed in ctx.get(ContextKey::Seeds) {
            let Some(request) = seed.payload::<WikidataRequest>() else {
                continue;
            };
                ...
                .await
            {
                ...
                Err(err) => {
                    tracing::warn!(
                        ...
                        error = %err,
                        "wikidata fetch failed; skipping seed"
                    );
                    continue;
                }
            };
            ...
            for (idx, observation) in response.records.into_iter().enumerate() {
                ...
                let payload_value = WikidataEntityPayload {
...
mod tests {
    use super::*;
    use crate::provider::StubWikidataProvider;
    ...
    fn suggestor_provenance_is_canonical() {
        ...
        let s = WikidataLookupSuggestor::new(Arc::new(StubWikidataProvider));
        assert_eq!(s.provenance(), WIKIDATA_PROVENANCE.provenance());
    }
    ...
    fn payload_family_and_version_are_stable() {
        ...
        assert_eq!(WikidataEntityPayload::FAMILY, "embassy.wikidata.entity");
        assert_eq!(WikidataEntityPayload::VERSION, 1);
    }
}
