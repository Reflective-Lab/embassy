...
//! Formation-callable surface — [`CrunchbaseLookupSuggestor`] reads
//! [`CrunchbaseRequest`] facts from `ContextKey::Seeds` and proposes
//! typed [`CrunchbaseOrganizationPayload`] facts to `ContextKey::Hypotheses`.
//!
...
use crate::provenance::CRUNCHBASE_PROVENANCE;
use crate::provider::{CrunchbaseProvider, CrunchbaseRequest};
...
pub struct CrunchbaseOrganizationPayload {
...
impl FactPayload for CrunchbaseOrganizationPayload {
    const FAMILY: &'static str = "embassy.crunchbase.organization";
    const VERSION: u16 = 1;
}

pub struct CrunchbaseLookupSuggestor<P: CrunchbaseProvider + 'static> {
    provider: Arc<P>,
}

impl<P: CrunchbaseProvider + 'static> CrunchbaseLookupSuggestor<P> {
...
#[async_trait]
impl<P: CrunchbaseProvider + 'static> Suggestor for CrunchbaseLookupSuggestor<P> {
    fn name(&self) -> &'static str {
        "CrunchbaseLookupSuggestor"
    }
    ...
    fn provenance(&self) -> &'static str {
        CRUNCHBASE_PROVENANCE.as_str()
    }

    fn accepts(&self, ctx: &dyn Context) -> bool {
        ctx.get(ContextKey::Seeds)
            .iter()
            .any(|fact| fact.payload::<CrunchbaseRequest>().is_some())
    }
    ...
    async fn execute(&self, ctx: &dyn Context) -> AgentEffect {
        ...
        for seed in ctx.get(ContextKey::Seeds) {
            let Some(request) = seed.payload::<CrunchbaseRequest>() else {
                continue;
            };
                ...
                .await
            {
                ...
                Err(err) => {
                    tracing::warn!(
                        ...
                        error = %err,
                        "crunchbase fetch failed; skipping seed"
                    );
                    continue;
                }
            };
            ...
            for (idx, observation) in response.records.into_iter().enumerate() {
                ...
                let payload_value = CrunchbaseOrganizationPayload {
...
mod tests {
    use super::*;
    use crate::provider::StubCrunchbaseProvider;
    ...
    fn suggestor_provenance_is_canonical() {
        ...
        let s = CrunchbaseLookupSuggestor::new(Arc::new(StubCrunchbaseProvider));
        assert_eq!(s.provenance(), CRUNCHBASE_PROVENANCE.as_str());
    }
    ...
    fn payload_family_and_version_are_stable() {
        ...
        assert_eq!(
            CrunchbaseOrganizationPayload::FAMILY,
            "embassy.crunchbase.organization"
        );
        assert_eq!(CrunchbaseOrganizationPayload::VERSION, 1);
    }
}
