...
//! Formation-callable surface — [`CompaniesHouseLookupSuggestor`] reads
//! [`CompaniesHouseRequest`] facts from `ContextKey::Seeds` and proposes
//! typed [`CompaniesHouseCompanyPayload`] facts to `ContextKey::Hypotheses`.
//!
...
use crate::provenance::COMPANIES_HOUSE_PROVENANCE;
use crate::provider::{CompaniesHouseProvider, CompaniesHouseRequest};
use crate::types::Company;

/// Typed fact payload — one [`Company`] per fact. Flattens the
...
pub struct CompaniesHouseCompanyPayload {
    pub company: Company,
...
impl FactPayload for CompaniesHouseCompanyPayload {
    const FAMILY: &'static str = "embassy.companies_house.company";
    const VERSION: u16 = 1;
}

pub struct CompaniesHouseLookupSuggestor<P: CompaniesHouseProvider + 'static> {
    provider: Arc<P>,
}

impl<P: CompaniesHouseProvider + 'static> CompaniesHouseLookupSuggestor<P> {
...
#[async_trait]
impl<P: CompaniesHouseProvider + 'static> Suggestor for CompaniesHouseLookupSuggestor<P> {
    fn name(&self) -> &'static str {
        "CompaniesHouseLookupSuggestor"
    }
    ...
    fn accepts(&self, ctx: &dyn Context) -> bool {
        ctx.get(ContextKey::Seeds)
            .iter()
            .any(|fact| fact.payload::<CompaniesHouseRequest>().is_some())
    }
    ...
    async fn execute(&self, ctx: &dyn Context) -> AgentEffect {
        ...
        for seed in ctx.get(ContextKey::Seeds) {
            let Some(request) = seed.payload::<CompaniesHouseRequest>() else {
                continue;
            };
                ...
                .await
            {
                ...
                Err(err) => {
                    tracing::warn!(
                        ...
                        error = %err,
                        "companies-house fetch failed; skipping seed"
                    );
                    continue;
                }
            };
            ...
            for (idx, observation) in response.records.into_iter().enumerate() {
                ...
                let payload_value = CompaniesHouseCompanyPayload {
...
mod tests {
    use super::*;
    use crate::provider::StubCompaniesHouseProvider;
    ...
    fn suggestor_provenance_is_canonical() {
        ...
        let s = CompaniesHouseLookupSuggestor::new(Arc::new(StubCompaniesHouseProvider));
        assert_eq!(s.provenance(), COMPANIES_HOUSE_PROVENANCE.provenance());
    }
    ...
    fn payload_family_and_version_are_stable() {
        ...
        assert_eq!(
            CompaniesHouseCompanyPayload::FAMILY,
            "embassy.companies_house.company"
        );
        assert_eq!(CompaniesHouseCompanyPayload::VERSION, 1);
    }
}
