...
//! across the sanctions trio (ofac-sls, eu-sanctions, commerce-csl).
//! Keep the field names identical across the three crates — Formations
...
impl SanctionsSubject {
    pub fn parse(name: impl AsRef<str>) -> Result<Self, OfacSlsError> {
        let name = name.as_ref().trim().to_string();
        if name.is_empty() {
            return Err(OfacSlsError::InvalidSubject("empty name".into()));
        }
        Ok(Self {
            name,
            country: None,
...
/// One hit on the OFAC sanctions list.
///
...
mod tests {
    ...
    fn subject_rejects_empty_name() {
        ...
        // live side and flood downstream review. Refuse at the
        // boundary.
