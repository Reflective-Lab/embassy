...
//! Formation-callable surface — [`OpenAlexLookupSuggestor`] reads
//! [`OpenAlexRequest`] facts from `ContextKey::Seeds` and proposes
//! typed [`OpenAlexWorkPayload`] facts to `ContextKey::Hypotheses`.
//!
...
use crate::provenance::OPENALEX_PROVENANCE;
use crate::provider::{OpenAlexProvider, OpenAlexRequest};
use crate::types::Work;

/// Typed fact payload — one [`Work`] per fact. Flattens the
...
pub struct OpenAlexWorkPayload {
    pub work: Work,
...
impl FactPayload for OpenAlexWorkPayload {
    const FAMILY: &'static str = "embassy.openalex.work";
    const VERSION: u16 = 1;
}

pub struct OpenAlexLookupSuggestor<P: OpenAlexProvider + 'static> {
    provider: Arc<P>,
}

impl<P: OpenAlexProvider + 'static> OpenAlexLookupSuggestor<P> {
...
#[async_trait]
impl<P: OpenAlexProvider + 'static> Suggestor for OpenAlexLookupSuggestor<P> {
    fn name(&self) -> &'static str {
        "OpenAlexLookupSuggestor"
    }
    ...
    fn provenance(&self) -> &'static str {
        OPENALEX_PROVENANCE.as_str()
    }

    fn accepts(&self, ctx: &dyn Context) -> bool {
        ctx.get(ContextKey::Seeds)
            .iter()
            .any(|fact| fact.payload::<OpenAlexRequest>().is_some())
    }
    ...
    async fn execute(&self, ctx: &dyn Context) -> AgentEffect {
        ...
        for seed in ctx.get(ContextKey::Seeds) {
            let Some(request) = seed.payload::<OpenAlexRequest>() else {
                continue;
            };
                ...
                .await
            {
                ...
                Err(err) => {
                    tracing::warn!(
                        ...
                        error = %err,
                        "openalex fetch failed; skipping seed"
                    );
                    continue;
                }
            };
            ...
            for (idx, observation) in response.records.into_iter().enumerate() {
                ...
                let payload_value = OpenAlexWorkPayload {
                    work: observation.content,
...
mod tests {
    use super::*;
    use crate::provider::StubOpenAlexProvider;
    ...
    fn suggestor_provenance_is_canonical() {
        ...
        let s = OpenAlexLookupSuggestor::new(Arc::new(StubOpenAlexProvider));
        assert_eq!(s.provenance(), OPENALEX_PROVENANCE.as_str());
    }
    ...
    fn payload_family_and_version_are_stable() {
        ...
        assert_eq!(OpenAlexWorkPayload::FAMILY, "embassy.openalex.work");
        assert_eq!(OpenAlexWorkPayload::VERSION, 1);
    }
}
