...
//! Formation-callable surface — [`ScbLookupSuggestor`] reads
//! [`ScbRequest`] facts from `ContextKey::Seeds` and proposes
//! typed [`ScbTableMetadataPayload`] facts to `ContextKey::Hypotheses`.
//!
...
use crate::provenance::SCB_PROVENANCE;
use crate::provider::{ScbProvider, ScbRequest};
use crate::types::TableMetadata;

/// Typed fact payload — one [`TableMetadata`] per fact. Flattens the
...
pub struct ScbTableMetadataPayload {
...
impl FactPayload for ScbTableMetadataPayload {
    const FAMILY: &'static str = "embassy.scb.table_metadata";
    const VERSION: u16 = 1;
}

pub struct ScbLookupSuggestor<P: ScbProvider + 'static> {
    provider: Arc<P>,
}

impl<P: ScbProvider + 'static> ScbLookupSuggestor<P> {
...
#[async_trait]
impl<P: ScbProvider + 'static> Suggestor for ScbLookupSuggestor<P> {
    ...
    fn accepts(&self, ctx: &dyn Context) -> bool {
        ctx.get(ContextKey::Seeds)
            .iter()
            .any(|fact| fact.payload::<ScbRequest>().is_some())
    }
    ...
    async fn execute(&self, ctx: &dyn Context) -> AgentEffect {
        ...
        for seed in ctx.get(ContextKey::Seeds) {
            let Some(request) = seed.payload::<ScbRequest>() else {
                continue;
            };
                ...
                .await
            {
                ...
                Err(err) => {
                    tracing::warn!(
                        ...
                        error = %err,
                        "scb fetch failed; skipping seed"
                    );
                    continue;
                }
            };
            ...
            for (idx, observation) in response.records.into_iter().enumerate() {
                ...
                let payload_value = ScbTableMetadataPayload {
                    table_metadata: observation.content,
...
mod tests {
    use super::*;
    use crate::provider::StubScbProvider;
    ...
    fn payload_family_and_version_are_stable() {
        ...
        assert_eq!(
            ScbTableMetadataPayload::FAMILY,
            "embassy.scb.table_metadata"
        );
        assert_eq!(ScbTableMetadataPayload::VERSION, 1);
    }
}
